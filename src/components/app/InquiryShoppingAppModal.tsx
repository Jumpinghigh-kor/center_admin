...
interface InquiryShoppingAppModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
  selectedInquiry: InquiryShoppingApp | null;
}

const InquiryShoppingAppModal: React.FC<InquiryShoppingAppModalProps> = ({
  isOpen,
  onClose,
  onSuccess,
  selectedInquiry,
                ...
                <div className="p-3 bg-gray-50 border border-gray-200 rounded-lg">
                  <div className="text-sm">
                    {selectedInquiry?.inquiry_type ? 
                      (commonCodeList.find(code => code.common_code === selectedInquiry.inquiry_type)?.common_code_name || selectedInquiry.inquiry_type) 
                      : "문의 유형 없음"
                    }
                  </div>
                </div>
...
export default InquiryShoppingAppModal;
