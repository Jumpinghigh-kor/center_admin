...
const NoticesShoppingAppList: React.FC = () => {
  const [noticesShoppingAppList, setNoticesShoppingAppList] = useState<NoticesShoppingApp[]>([]);
  const { checkedItems, allChecked, handleAllCheck, handleIndividualCheck, resetCheckedItems } = useCheckbox(noticesShoppingAppList.length);
  const user = useUserStore((state) => state.user);
  const navigate = useNavigate();

  // 페이지네이션 훅 사용
  const pagination = usePagination({
    totalItems: noticesShoppingAppList.length,
    itemsPerPage: 10,
  });
  
  // 현재 페이지에 표시할 데이터
  const currentNoticesShoppingAppList = pagination.getCurrentPageData(noticesShoppingAppList);

  // 공지사항 목록 불러오기
  const getNoticesShoppingAppList = async (searchParams?: any) => {
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}/app/noticesShoppingApp/selectNoticesShoppingAppList`,
        {
          ...searchParams
        }
      );

      setNoticesShoppingAppList(response.data.result || response.data || []);
      resetCheckedItems();
  ...
  const { searchData, setSearchData, handleSearch, handleReset } = useSearch({
    onSearch: getNoticesShoppingAppList,
    initialSearchData: {
      title: "",
      notices_type: "",
      start_dt: "",
  ...
  const handleBatchDelete = async () => {
    const selectedNotices = checkedItems
      .map((checked, index) => (checked ? noticesShoppingAppList[index] : null))
      .filter((notice): notice is NoticesShoppingApp => notice !== null)
      .map(notice => notice.notices_shopping_app_id);
    ...
    if (window.confirm("선택한 공지사항들을 삭제하시겠습니까?")) {
      try {
        await axios.post(
          ...
          {
            notices_shopping_app_ids: selectedNotices,
            userId: user.index,
          }
        );

        // 목록 새로고침
        getNoticesShoppingAppList();
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={() => navigate('/app/noticeShopping/noticesShoppingAppRegister')}
        ...
        {noticesShoppingAppList?.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <div className="flex justify-between items-center mb-2">
              <p className="text-sm font-semibold">총 {noticesShoppingAppList.length}건</p>
            ...
            <table className="min-w-full bg-white">
              ...
              <tbody>
                {currentNoticesShoppingAppList.map((notice, index) => (
                  <tr
                    key={notice.notices_shopping_app_id}
                    className="h-16 border-b border-gray-200 hover:bg-gray-50 cursor-pointer"
                    onClick={() => navigate(`/app/noticeShopping/noticesShoppingAppDetail?noticesShoppingAppId=${notice.notices_shopping_app_id}`)}
                    onDoubleClick={() => {
                      navigate(`/app/noticeShopping/noticesShoppingAppRegister?noticesShoppingAppId=${notice.notices_shopping_app_id}`);
                    }}
                  >
                    <td
                    ...
                    <td className="pl-4 text-center">{noticesShoppingAppList?.length - index}</td>
...
export default NoticesShoppingAppList;
