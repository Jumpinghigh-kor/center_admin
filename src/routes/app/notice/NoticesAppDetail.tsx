...
const NoticesAppDetail: React.FC = () => {
  const user = useUserStore((state) => state.user);
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const [noticesData, setNoticesData] = useState<NoticesApp | null>(null);
  ...
  const getNoticesAppDetail = async () => {
    if (!noticesAppId) return;
    
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}/app/noticesApp/selectNoticesAppDetail`,
        {
          notices_app_id: noticesAppId
        }
      );
  ...
  const handleUpdate = async () => {
    ...
    try {
      await axios.post(
        ...
        {
          noticesAppId: noticesData.notices_app_id,
    ...
    if (window.confirm("정말로 삭제하시겠습니까?")) {
      try {
        await axios.post(
          ...
          {
            notices_app_ids: [noticesData?.notices_app_id],
            userId: user.index,
          }
        );
...
export default NoticesAppDetail;
